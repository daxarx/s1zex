...
class TelegramLogsHandler(logging.Handler):
    ...
    def _gen_web_debug_button(self, item: Exception) -> list:
        if not item.sysinfo:
            return []
    ...
    async def _start_debugger(self, call: "InlineCall", item: Exception):  # type: ignore
        ...
        async with self._send_lock:
            ...
            for client_id in self._mods:
                ...
                if len(self._queue[client_id]) > 5:
                    ...
                    logfile.name = "adaptator-logs.txt"
                    logfile.seek(0)

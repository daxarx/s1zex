...
from .types import ReplyMarkup, ListLike, Module
...
async def answer(
    ...
    **kwargs,
    ...
    :param message: Message to answer to. Can be a tl message or adaptator inline object
...
async def convert_folders(client):
    ...
    try:
        folder = next(folder for folder in folders if hasattr(folder, "title") and folder.title == "s1zex")
    except Exception:
        folder = None

    if folder is not None:
        folder.title = "adaptator"
...
async def asset_channel(
    ...
    *,
    ...
    # legacy adaptator / s1zex chats conversion to adaptator
    if title.startswith("s1zex-"):
        title = title.replace("s1zex-", "adaptator-")

    async for d in client.iter_dialogs():
        if (d.title == title) or ((d.title.replace("s1zex-", "adaptator-") == title) if d.title.startswith("s1zex-") else False):
    ...
    if _folder:
        ...
        _folder = "adaptator" if _folder == "s1zex" else _folder

        if _folder != "adaptator":
            raise NotImplementedError

        folders = await client(GetDialogFiltersRequest())

        try:
            folder = next(folder for folder in folders if hasattr(folder, "title") and folder.title == "adaptator")
        except Exception:
            folder = None
...
def get_platform_emoji(client: typing.Optional[CustomTelegramClient] = None) -> str:
    ...
    """
    BASE = (
        "<emoji document_id=5281005969420066218>🥀</emoji> <b>adaptator</b>",
    )
...
def get_git_hash() -> typing.Union[str, bool]:
    """
    Get current adaptator git hash
...
def get_commit_url() -> str:
    """
    Get current adaptator git commit url
    :return: Git commit url
    """
    try:
        hash_ = get_git_hash()
        return (
            f'<a href="https://github.com/s1zexxx/Adaptator/commit/{hash_}">#{hash_[:7]}</a>'
        )
    except Exception:
        return "Unknown"
...
def get_git_info() -> typing.Tuple[str, str]:
    ...
    return (
        hash_,
        f"https://github.com/s1zexxx/Adaptator/commit/{hash_}" if hash_ else "",
    )

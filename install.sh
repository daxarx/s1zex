...
runin() {
	...
	{ "$@" 2>>../adaptator-install.log || return $?; } | while read -r line; do
		printf "%s\n" "$line" >>../adaptator-install.log
	done
}
...
printf "\n\e[1;35;47m                   \e[0m"
printf "\n\e[1;35;47m ADAPTATOR ADAPTATOR \e[0m"
printf "\n\e[1;35;47m ADAPTATOR ADAPTATOR \e[0m"
printf "\n\e[1;35;47m ADAPTATOR ADAPTATOR \e[0m"
...
if [ ! x"$SUDO_USER" = x"" ]; then
	chown "$SUDO_USER:" adaptator-install.log
fi

if [ -d "Adaptator/s1zex" ]; then
	cd Adaptator || {
...
if [ -f ".setup_complete" ]; then
	# If Adaptator is already installed by this script
	PYVER=""
...
echo "Installing..." >adaptator-install.log
...
if echo "$OSTYPE" | grep -qE '^linux-gnu.*' && [ -f '/etc/debian_version' ]; then
	...
	printf "\r\033[1;31mUnrecognised OS.\e[0m Please follow 'Manual installation' at \033[0;94mhttps://github.com/s1zexxx/Adaptator/#-installation\e[0m"
	exit 1
fi
...
${SUDO_CMD}rm -rf Adaptator
# shellcheck disable=SC2086
runout ${SUDO_CMD}git clone https://github.com/s1zexxx/Adaptator || {
